...
# 背景
wait_image =  pg.image.load("arc'sreminder.jpg").convert()
    ...
    for event in pg.event.get():
        ...
        if event.type == pg.KEYDOWN:
            for note in notes1 + notes2:
                ...
                if event.key == getattr(pg, 'K_' + note['key']) and 500 <= note['y'] <= 575:
                    if note in notes1:
                        notes1.remove(note)
                    elif note in notes2:
                        notes2.remove(note)
        ...
        for note in notes1 + notes2:
            ...
            if note['y'] > height:
                ...
                if note['key'] == 'f':
                    ...
                    pg.draw.line(screen, light_purple, (note['x'], note['y']), (note['x'], note['y'] + 15), 40)
                elif note['key'] == 'h':
                    pg.draw.line(screen, purple, (note['x'], note['y']), (note['x'], note['y'] + 15), 40)
                elif note['key'] == 'j':
                    pg.draw.line(screen, (255, 0, 0), (note['x'], note['y']), (note['x'], note['y'] + 15), 40)
